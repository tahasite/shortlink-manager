...
if ($result->num_rows > 0) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['original_links'])) {
            ...
            if ($short_code_method == 'fixed') {
                ...
                foreach ($links as $index => $original_link) {
                    ...
                    if ($result_check->num_rows > 0) {
                        echo json_encode(['error' => "کد کوتاه وارد شده قبلا وجود دارد. لطفا کد دیگری وارد کنید."]);
                        exit;
                    } else {
            ...
            if ($short_code_method == 'manual') {
                ...
                if (count($links) !== count($manual_codes)) {
                    echo json_encode(['error' => 'تعداد لینک‌ها و کدهای کوتاه وارد شده باید برابر باشند.']);
                    exit;
                }
            }
            foreach ($links as $index => $original_link) {
                ...
                if ($short_code_method == 'manual') {
                    if (isset($manual_codes[$index])) {
                        ...
                        echo json_encode(['error' => "لطفا کد کوتاه برای لینک شماره " . ($index + 1) . " وارد کنید."]);
                        exit;
                    }
    ...
    echo "اطلاعاتی برای دریافت وجود ندارد.";
}
